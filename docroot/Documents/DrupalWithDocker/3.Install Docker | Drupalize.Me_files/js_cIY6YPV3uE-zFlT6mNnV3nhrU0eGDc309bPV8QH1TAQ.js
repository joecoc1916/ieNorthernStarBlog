...
/**
 * @file
 * Javascript for tutorials
 */
(function($) {
  /**
   * @fileOverview drupalizetutorial.js
   *
   * Handle the enlarge/collapse behavior of the video player when
   * embedded in a breaker within a tutorial.
   *
   */
   Drupal.behaviors.dmeTutorial = {
     attach: function(context) {

       $('.tutorial-video-thumbnail', context).click(function(event) {
         // Find the video id of the clicked element.
         var videoId = $(this).data('video');
         // Make the player go full width.
         var $thumbnail = $('#screenshot-' + videoId);

         $thumbnail.parents('div.tutorial--breaker').addClass('tutorial--breaker-expanded');

         // Hide any text in the breaker.
         $thumbnail.parents('div.tutorial--breaker').find('div.tutorial--breaker-right').hide();
         // Hide the screenshot.
         $thumbnail.hide();
         // Show the player, and start the video.
         $('#embeddedvideo-' + videoId).show();
         jwplayer('drupalizeme-video-player').play();
         event.preventDefault();
         return false;
       });

       $('.tutorial-video-close', context).click(function(event) {
         var videoId = $(this).data('video');
         // stop the video (may already be happening)
         jwplayer('drupalizeme-video-player').stop();
         // set the display none on the video div.
         $('#embeddedvideo-' + videoId).hide();
         // show the screen shot + heading / paragraph text.
         var $thumbnail = $('#screenshot-' + videoId);
         $thumbnail.parents('div.tutorial--breaker').removeClass('tutorial--breaker-expanded');
         $thumbnail.parents('div.tutorial--breaker').find('div.tutorial--breaker-right').show();
         $thumbnail.show();
       ...
       $('.tutorial-video-thumbnail', context).once('expand-tutorial', function() {
         // Find the video id of the first video player element.
         var videoId = $(this).data('video');
         // Make the player go full width.
         var $thumbnail = $('#screenshot-' + videoId);
